...
import {
  ...
  atualizaUsuariosOnlineSuccess,
  atualizaUsuariosOnlineFailed,
  CriarGameFailed,
  CriarGameSuccess,
} from './actions';
...
export function* AtualizaUsuarriosOnline(payload) {
  try {
    yield put(atualizaUsuariosOnlineSuccess(payload));
    payload?.payload?.callback?.();
  } catch (error) {
    yield put(atualizaUsuariosOnlineFailed(error));
    yield put(RequestFailed(error));
  }
}
...
export default all([
  ...
  takeLatest(ATUALIZA_USUARIOS_ONLINE_REQUEST, AtualizaUsuarriosOnline),
